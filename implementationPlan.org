...
- dotenvx for env vars and secrets (see [[file:ADR/0004-dotenvx-for-env-vars.org][ADR 0004]])
- XState for state management (see [[file:ADR/0005-xstate-for-state-management.org][ADR 0005]])
...
│   └── machines/
│       └── bookStatusMachine.ts # Book lifecycle state machine (XState)
...
- Install dotenvx (=npm install @dotenvx/dotenvx --save-dev=)
- Set initial env vars (=npx dotenvx set DATABASE_PATH ./books.db=)
- Add =.env.keys= to =.gitignore=
- Update =dev= script to use =dotenvx run --= prefix
...
- Create =src/machines/bookStatusMachine.ts= :: XState machine for
  book lifecycle (=toRead= → =reading= → =done=)
- Create server functions (=createServerFn=) for all CRUD operations
- Status transitions go through the XState machine, not ad-hoc string
  assignment
...
*** Phase 6: Polish
- Status toggle on the list uses XState =send()= — only valid
...
#+begin_src shell
  ...
  # Run the app (dotenvx decrypts env vars automatically via dev script)
  npm run dev
#+end_src

The =dev= script wraps =dotenvx run --= so env vars are decrypted
transparently. App runs on =http://localhost:3000= (single process, no
separate API server).
