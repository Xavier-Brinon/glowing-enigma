...
| Runbook (shared)     | =runBook.org=                              | How to use org-drill (one for all topics) |
...
#+begin_src org
For setup and daily workflow, see [[./runBook.org][Spaced Repetition Runbook]].
#+end_src

The runbook (=runBook.org=) is shared across all topics. There is one
...
|------------------+-------------------------------------------+---------------------------------------------------|
| Broadening doc   | =FollowUp_[concept].org=                  | =FollowUp_resource_management.org=                |
| Assessment doc   | =teaching_[concept]_assessment.org=       | =teaching_resource_management_assessment.org=     |
| Runbook          | =runBook.org= (shared, one for all)       | =runBook.org=                                     |

Note: the first concrete broadening doc was named =FollowUpKnowledge.org=
before this convention was established. New documents should follow the
=FollowUp_[concept].org= pattern.
...
GUIDE HIERARCHY
  ...
  └──▶ goDeeperStyleguide.org  (this document)
       ...
       └──▶ runBook.org             (shared learner runbook)
#+end_example
