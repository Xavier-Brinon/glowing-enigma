g+title: Working with OpenSpec with multiple Claude profiles
#+author: HaQadosch
#+date: [2026-02-15 Sun]
#+startup: indent
...
#+begin_src bash
  Created: design.md
  Progress: 2/4 — design done, tasks now only blocked by specs.
  [x] proposal
  [x] design
  [ ] specs     ← next
...
#+begin_src bash
  ...
  Progress: 3/4 — only tasks remaining, now unblocked.
...
Seems a bit shipish and vague. We'll see 🤷🏻.
** opsx:continue -> task
Before starting, I want to make it very cleare to *Claude* and *OPSX*
that any new task must be done from a fresh new branch from origin
main and pulled into a new git worktree.

There is also a new TanStack CLI that I want to see being used. I'd
like to have the linter, formatter from [[https://oxc.rs/][OXC]] to replace the classic
eslint and prettier.
So I'm asking *Claude* to update the docs accordingly and create an [[file:ADR/0006-oxc-for-linting-and-formatting.org][ADR]].

#+begin_src bash
  /opsx:continue
#+end_src
This creates a [[file:openspec/changes/scaffold-and-pipes/tasks.md][tasks.md]] file.
I can review each of them in case I need to adjust any one of them.
...
The openspec created 7 tasks:
- 0. Git Setup :: fetch origin, create worktree + branch from
  origin/main
- 1. Project Scaffold :: TanStack Start CLI, strip boilerplate, verify
  strict TypeScript, pin .nvmrc to 24.13, typecheck passes
- 2. Oxlint + Oxfmt :: install both, wire the three npm scripts (lint,
  format, format:check), confirm both pass
- 3. dotenvx :: install + prefix npm run dev, set DATABASE_PATH,
  confirm .env.keys is gitignored
- 4. Database Connection :: write failing test first, create
  src/lib/db.ts with error guard, test passes, commit
- 5. Vitest Smoke Test :: confirm test runner is wired, at least one
  passing test
- 6. Default Page :: __root.tsx renders a placeholder, npm run dev
  serves it cleanly at localhost:3000
- 7. Pre-Commit Verification :: typecheck + lint + format:check +
  tests all pass, final commit

I'll have to make sure that the TanStack CLI is being used correctly
at step 1.
** opsx:apply -> 1 step at a time
It tries to do all of them 😠.
Update the schemas to re-inforce the stop gap.
** TODO create the github workflow
to confirm all tests are passing and app is building:
- Needs fast effeective validation
- Type checking
- Lint Style checks
- Coding standards
...
💡Check for behaviour not syntax.
** TODO Ensure that the apply steps does
- work in small steps
  - Avoid large-batch changes (too big merge)
  - Collect feedback faster
  - Evaluate at every change
- Optimise for fast feedback
  - Faster than the rate of change (the faster you go the more checks you need)
  - Feedback must be fast and accurate
- Tests are the only source of truth
  - Tests are definitive for release; not the manual check or LGTM
- Avoid feature branches
  - Don't hide changes, or don't keep them in a branch for too long
  - Everyone should integrate continuously
- Invest in deployment pipeline
  - Make it fast, reliable, and repeatable (reproducible ?)
  - All the way to production

** Update the Design to avoid feature branch
I should ensure this is trunk based and each steps in ~/opsx:apply~
should be merged to main, not each feature. As for the CICD, this app
is mainly for local users. Not meant to be on the web.

Create new [[file:ADR/0007-trunk-based-development.org][ADR]] to put TBD in place.
